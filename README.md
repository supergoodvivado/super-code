...
- 主要功能：16 种商品、库存管理、缺货检测、管理员补货与改价、数量选择、最多两种商品、投币、出货、手动逐元找零、数码管显示、LED 状态指示、蜂鸣器提示
...
将 SW1-SW4 拨成 `1110`，长按 KEY4 约 2 秒进入管理员模式。KEY1 在商品选择页和库存查看页之间切换，KEY2 为当前商品补货 1 件（上限 15），KEY3 进入价格设置页，拨码 `0000`-`1111` 对应 0-15 元，按 KEY1 保存；KEY4 退出。正常复位仍使用 `SW=1111 + KEY4`。

库存每种商品初始 5 件。只有成功出货才扣库存；库存不足时订单不会提交并产生缺货提示。出货后如有余额进入找零状态：KEY3 退 1 元，KEY2 配合拨码退 5、10 或 20 元。
